  ...
  describe('key', () => {
    describe('base', () => {
      ...
      it('resolves using the config', async () => {
        ...
        await waitFor(() => {
          expect(container.firstChild.textContent).toEqual(`Block Number, ${mockData}`);
        });
      });
      ...
      it('resolves using the context', async () => {
        ...
        mockedEthFetcher.mockImplementation(jest.fn(() => jest.fn().mockReturnValue(mockData)));
    ...
    describe('contract', () => {
      ...
      it('throws ABI Missing', async () => {
        ...
        expect(() => {
          ...
          new ABINotFound('Missing ABI for 0x6b175474e89094c44da98b954eedeac495271d0f'),
        );
      });
      ...
      it('resolves using the fetcher passed', async () => {
        ...
        function Page() {
          const { data, isValidating } = useEthSWR(['0x111', 'balanceOf', '0x01'], loadData, {
            dedupingInterval: 0,
          });
      ...
      it('resolves multiple results', async () => {
        ...
        function Page() {
          const { data: balances, error, isValidating } = useEthSWR<BigNumber[]>(
            multiKeys,
            loadData,
          );
  ...
  describe('subscribe', () => {
    describe('base', () => {
      ...
      it('listens an event and update data', async () => {
        ...
        const keyResolver = jest.fn().mockReturnValueOnce(initialData).mockReturnValue(finalData);
        ...
        await waitFor(() => {
          expect(container.firstChild.textContent).toEqual(`Balance, ${finalData}`);
      ...
      it('listens a list of events an update data', async () => {
        ...
        await waitFor(() => {
          expect(container.firstChild.textContent).toEqual(`Balance, ${initialData + 10}`);
          expect(keyResolver).toHaveBeenNthCalledWith(1, method);
          expect(keyResolver).toHaveBeenNthCalledWith(2, method);
          expect(mockedLibrary.listenerCount('block')).toEqual(1);
        });
      ...
      it('listens a list of events and invoke the callback', async () => {
        ...
        await waitFor(() =>
          expect(container.firstChild.textContent).toEqual(`Balance, ${initialData}`),
        );
          ...
          const { data, mutate } = useEthSWR(
            ['0x6126A4C0Eb7822C12Bea32327f1706F035b414bf', 'balanceOf', account],
            {
              ...
              subscribe: {
                name: 'Transfer',
                topics: [null, account],
                on: callback.mockImplementation((data, fromAddress, toAddress, amount, event) => {
