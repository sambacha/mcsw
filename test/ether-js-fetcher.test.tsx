...
describe('ethFetcher', () => {
  ...
  describe('eth', () => {
    it('return the value', async () => {
      ...
      const library = new Web3ProviderMock(jsonRpcFetchFunc);
      const balance = 1;
      library.getBalance = jest.fn().mockImplementation(() => Promise.resolve(balance));
    ...
    it('returns the values', async () => {
      ...
      const fetcher = etherJsFetcher(library);
      // SWR spread the array when it invoke the fetcher
      await expect(fetcher(JSON.stringify([['getBalance']]))).resolves.toEqual([balance]);
    });
  });
  ...
  describe('contract', () => {
    ...
    it('return the value', async () => {
      ...
      // SWR spread the array when it invoke the fetcher
      await expect(fetcher(...[contract, 'balanceOf', account])).resolves.toEqual(balance);
    });
    it('return the values', async () => {
      ...
      const fetcher = etherJsFetcher(library, ABIs);
      // SWR spread the array when it invoke the fetcher
      await expect(fetcher(JSON.stringify([[contract, 'balanceOf', account]]))).resolves.toEqual([
        balance,
      ]);
    });
  });
